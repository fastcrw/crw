...
///     `SizeHint::default()` — i.e. no exact value — so `content_length()` is
...
/// over the cap, and — because the cap is applied to the bytes this function
/// yields — it is measured AFTER decompression, which is the point.
///
...
/// has `read_capped` — and hoisting one copy into `crw-core` (which already
